...
var ValidatorMixin = {
  ...
  tryRemoveArrays: function (xpath, namespace, type, newValue) {
    var self = this;
      ...
      try {
        if (type[i].anyChildren) {
          ...
          _.each(value, function (child, name) {
            ...
            var childName = self.namespacedName(namespace, name);
            ...
            else if (!self.resolveElement(xpath, type[i].children[childName]).isArray) {
              assert(child.length === 1, child);
              value[name] = child[0];
            }
          });
  ...
  resolveElementTypeName: function (xpath, nodeAttributes, element) {
    var self = this;
  ...
  nodeNamespace: function (node) {
    ...
    if (!self.namespacePrefixes[node[self.xmlnskey].uri]) {
      throw new xml2js.ValidationError("Unknown namespace " + node[self.xmlnskey].uri + ", element: " + util.inspect(node, false, null));
    }
    return self.namespacePrefixes[node[self.xmlnskey].uri];
  },

  // Similar to xsd.namespacedName, just without xsPrefix argument
  namespacedName: function (namespace, name) {
    var self = this;
  ...
  namespacedOrNotName: function (namespace, name, namespaced) {
    var self = this;

    if (namespaced) {
      return self.namespacedName(namespace, name);
    }
    else {
      return name.replace(/^[^:]+:/, '');
    }
  ...
  createNamespacedPath: function (stack, xpath, newValue) {
    ...
    _.each(stack, function (node) {
      var namespace = self.nodeNamespace(node);
      path.push(self.namespacedName(namespace, node['#name']));
    });
    // We get the name of the last node from the last element of the xpath
    var splitXpath = xpath.split('/');
    path.push(self.namespacedName(self.nodeNamespace(newValue), splitXpath[splitXpath.length - 1]));
    return path;
  },

  normalizeNamespaces: function (namespace, value, namespaced) {
    var self = this;
    ...
    _.each(value, function (val, key) {
      ...
      delete value[key];
      value[self.namespacedOrNotName(namespace, key, namespaced)] = val;
    });
  }
};
...
function validator(xpath, currentValue, newValue, stack) {
  ...
  var namespace = parser.nodeNamespace(newValue);
  ...
  _.each(path.slice(0, path.length - 1), function (segment, i) {
    if (!currentElementSet[segment]) {
    ...
    else if (!parser.resolveElementTypeName(xpath, stack[i][parser.attrkey], currentElementSet[segment])) {
      ...
      var type = parser.resolveType(xpath, parser.resolveElementTypeName(xpath, stack[i][parser.attrkey], currentElementSet[segment]));
  ...
  var lastSegmentTypeName = parser.resolveElementTypeName(xpath, newValue[parser.attrkey], currentElementSet[lastSegment]);

  if (newValue[parser.attrkey]) {
    ...
    _.each(newValue[parser.attrkey], function (value, attribute) {
      var attributeName = parser.namespacedName(namespace, attribute);
      ...
      // TODO: xsi prefix should probably not be hard-coded
        ...
        if (_.isString(value)) {
          delete newValue[parser.attrkey][attribute];
          newValue[parser.attrkey][parser.namespacedOrNotName(namespace, attribute, outputWithNamespace)] = parser.tryParse(parse, value);
        }
        else if (value.value) {
          // TODO: What if user wants namespace information, we should not replace with only the value in that case
          delete newValue[parser.attrkey][attribute];
          newValue[parser.attrkey][parser.namespacedOrNotName(namespace, attribute, outputWithNamespace)] = parser.tryParse(parse, value.value);
        }
        else {
    ...
    if (_.isEmpty(newValue[parser.attrkey])) {
  ...
  if (parse.length !== 0) {
    ...
    else if (_.isEmpty(_.without(_.keys(newValue), parser.charkey)) && newValue[parser.charkey]) {
    ...
    else if (_.isEmpty(_.without(_.keys(newValue), parser.charkey, parser.attrkey, parser.xmlnskey)) && newValue[parser.charkey]) {
    ...
    newValue = parser.tryRemoveArrays(xpath, namespace, type, newValue);
    parser.normalizeNamespaces(namespace, newValue, outputWithNamespace);
  }
