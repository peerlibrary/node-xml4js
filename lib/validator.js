...
function validator(xpath, currentValue, newValue, stack) {
  ...
  _.each(newValue[parser.attrkey] || {}, function (value, attribute) {
    ...
    // TODO: xsi prefix should probably not be hard-coded
      ...
      if (_.isString(value)) {
        delete newValue[parser.attrkey][attribute];
        newValue[parser.attrkey][parser.namespacedOrNotName(namespace, attribute, options.outputWithNamespace)] = parser.tryParse(parse, value);
      }
      else if (value.value) {
        // TODO: What if user wants namespace information, we should not replace with only the value in that case
        delete newValue[parser.attrkey][attribute];
        newValue[parser.attrkey][parser.namespacedOrNotName(namespace, attribute, options.outputWithNamespace)] = parser.tryParse(parse, value.value);
      }
      else {
  ...
  if (parse.length !== 0) {
    ...
    else if (_.isEmpty(_.without(_.keys(newValue), parser.charkey))) {
    ...
    parser.normalizeNamespaces(namespace, newValue, options.outputWithNamespace);
  }
