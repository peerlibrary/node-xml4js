...
XsdSchema.prototype.namespacedName = function (name) {
  ...
  if (self.xsPrefix && xsPrefixRegex.test(name)) {
...
XsdSchema.prototype.parseTypesChoice = function (input) {
  var self = this;

  assert(input[self.xsPrefix + 'choice'], input);
  var children = {};
  assert(input[self.xsPrefix + 'choice'].length === 1, input[self.xsPrefix + 'choice']);
  var isArrayDefault = null;
  if (input[self.xsPrefix + 'choice'][0].$) {
    if (input[self.xsPrefix + 'choice'][0].$.maxOccurs) {
      isArrayDefault = input[self.xsPrefix + 'choice'][0].$.maxOccurs === 'unbounded' || parseInt(input[self.xsPrefix + 'choice'][0].$.maxOccurs) > 1;
    }
    delete input[self.xsPrefix + 'choice'][0].$.minOccurs;
    delete input[self.xsPrefix + 'choice'][0].$.maxOccurs;
    assert(_.isEmpty(input[self.xsPrefix + 'choice'][0].$), input[self.xsPrefix + 'choice'][0].$);
  }
  delete input[self.xsPrefix + 'choice'][0].$;
  if (input[self.xsPrefix + 'choice'][0][self.xsPrefix + 'element']) {
    _.extend(children, self.parseElements(input[self.xsPrefix + 'choice'][0], isArrayDefault));
  }
  assert(_.isEmpty(input[self.xsPrefix + 'choice'][0]), input[self.xsPrefix + 'choice'][0]);
...
XsdSchema.prototype.parseTypesSequence = function (input) {
  ...
  assert(input[self.xsPrefix + 'sequence'], input);
  var type = {};
  var children = {};
  assert(input[self.xsPrefix + 'sequence'].length === 1, input[self.xsPrefix + 'sequence']);
  var isArrayDefault = null;
  if (input[self.xsPrefix + 'sequence'][0].$) {
    if (input[self.xsPrefix + 'sequence'][0].$.maxOccurs) {
      isArrayDefault = input[self.xsPrefix + 'sequence'][0].$.maxOccurs === 'unbounded' || parseInt(input[self.xsPrefix + 'sequence'][0].$.maxOccurs) > 1;
    }
    delete input[self.xsPrefix + 'sequence'][0].$.minOccurs;
    delete input[self.xsPrefix + 'sequence'][0].$.maxOccurs;
    assert(_.isEmpty(input[self.xsPrefix + 'sequence'][0].$), input[self.xsPrefix + 'sequence'][0].$);
  }
  delete input[self.xsPrefix + 'sequence'][0].$;
  if (input[self.xsPrefix + 'sequence'][0][self.xsPrefix + 'element']) {
    _.extend(children, self.parseElements(input[self.xsPrefix + 'sequence'][0], isArrayDefault));
  }
  if (input[self.xsPrefix + 'sequence'][0][self.xsPrefix + 'choice']) {
    _.extend(children, self.parseTypesChoice(input[self.xsPrefix + 'sequence'][0]));
  }
  if (input[self.xsPrefix + 'sequence'][0][self.xsPrefix + 'any']) {
    assert(input[self.xsPrefix + 'sequence'][0][self.xsPrefix + 'any'].length === 1, input[self.xsPrefix + 'sequence'][0][self.xsPrefix + 'any']);
    type.anyChildren = true;
    var isArray = isArrayDefault;
    if (input[self.xsPrefix + 'sequence'][0][self.xsPrefix + 'any'][0].$.maxOccurs) {
      isArray = input[self.xsPrefix + 'sequence'][0][self.xsPrefix + 'any'][0].$.maxOccurs === 'unbounded' || parseInt(input[self.xsPrefix + 'sequence'][0][self.xsPrefix + 'any'][0].$.maxOccurs) > 1;
    }
    if (_.isBoolean(isArray)) {
      type.isArray = isArray;
    }
  }
  delete input[self.xsPrefix + 'sequence'][0][self.xsPrefix + 'any'];
  assert(_.isEmpty(input[self.xsPrefix + 'sequence'][0]), input[self.xsPrefix + 'sequence'][0]);
...
XsdSchema.prototype.parseSimpleType = function (input) {
  ...
  _.each(input[self.xsPrefix + 'simpleType'] || [], function (simpleType) {
    var type = {};
    assert(!(simpleType[self.xsPrefix + 'restriction'] && simpleType[self.xsPrefix + 'union']), simpleType);
    if (simpleType[self.xsPrefix + 'restriction']) {
      ...
      if (simpleType[self.xsPrefix + 'restriction'][0].$.base !== 'anySimpleType') {
        type.base = self.namespacedTypeName(simpleType[self.xsPrefix + 'restriction'][0].$.base);
      }
      delete simpleType[self.xsPrefix + 'restriction'][0].$.base;
      assert(_.isEmpty(simpleType[self.xsPrefix + 'restriction'][0].$), simpleType[self.xsPrefix + 'restriction'][0].$);
      delete simpleType[self.xsPrefix + 'restriction'][0].$;
    ...
    if (simpleType[self.xsPrefix + 'union']) {
      ...
      type.base = _.map(simpleType[self.xsPrefix + 'union'][0].$.memberTypes.split(/\s+/), function (base) {
        return self.namespacedTypeName(base);
      });
      delete simpleType[self.xsPrefix + 'union'][0].$.memberTypes;
      assert(_.isEmpty(simpleType[self.xsPrefix + 'union'][0].$), simpleType[self.xsPrefix + 'union'][0].$);
      delete simpleType[self.xsPrefix + 'union'][0].$;
      assert(_.isEmpty(simpleType[self.xsPrefix + 'union'][0]), simpleType[self.xsPrefix + 'union'][0]);
    }
